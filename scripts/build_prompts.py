...
FEMME = """FEMALE FIGURE DIRECTIVE (mandatory for every woman in the scene): render her with heightened yet tasteful sensuality — a mature adult woman, confident and entirely at ease in her own skin. Favour: a long bare back with the line of the spine caught by low golden light, bare shoulders with silk or wet gauze slipping off one of them, the curve of waist and hip clearly drawn, a hand lifted into loosened hair, an arched or reclining posture. Give her a languid, alive expression — parted lips, heavy-lidded eyes meeting the viewer or a slow sidelong glance. Her build: {BUILD}. Her hair: {HAIR}. Drapery is silk or sheer gauze that clings and reveals the body's line rather than hiding it. Nudity is fine-art: bare breasts, back and hips may be shown as soft classical anatomy, painterly and never graphic or clinical. {ALLURE}
...
DEFAULT_HAIR = "long and loose, softly waved, falling down her back"

WING = """WING LOCK (mandatory): wings belong to a figure's back — they are never a separate prop,
...
def wing_lock(c):
    """Chỉ thêm WING LOCK cho lá nào cảnh có cánh / thiên thần."""
    txt = c.get("scene", "").lower()
    return WING if re.search(r"\bwing|angel|archangel|cherub", txt) else ""
...
def build(c):
    ...
    return (tpl
            ...
            .replace("{FEMME}", (FEMME.replace("{BUILD}", c.get("build", DEFAULT_BUILD))
                                 ...
                                 .replace("{HAIR}", c.get("hair", DEFAULT_HAIR))
